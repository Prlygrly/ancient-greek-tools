...
function buildPrompt(b) {
  ...
  lines.push('- better: if a more natural or more correct Ancient Greek phrasing exists AT THE STUDENT’S LEVEL, give it and explain in one sentence why it is better. Stay within the vocabulary the module guidance says the student knows, preferring words already visible in the question, the model answers, and the student’s own answer. If a genuinely better phrasing requires a word they may not know, you may use it ONLY with an inline gloss and flag, e.g. Ἕλλην (= a Greek; new word). If nothing better can be said at their level, return an empty string.');
